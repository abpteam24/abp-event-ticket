<?php
	...
	add_action( 'abpet_slider_template', function ( $img_ids = [], $abpet_slider = [] ) {
        if ( empty( $img_ids ) || ! is_array( $img_ids ) ) {
            return;
        }
		$total_images        = count( $img_ids );
		$slider_indicator    = ( $abpet_slider['indicator_visible'] ?? 'on' ) ?: 'on';
		$indication_position = ( $abpet_slider['indication_position'] ?? 'bottom' ) ?: 'bottom';
		$indication_position = ( $slider_indicator === 'on' ) ? $indication_position : '';
		$popup_id            = uniqid( '#abpet_slider_' );
		$active_popup        = ( $abpet_slider['visible_popup'] ?? 'on' ) ?: 'on';
		?>
        <div class="abpet_slider">
