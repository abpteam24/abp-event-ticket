<?php
    ...
    add_action('abpet_total_price_template', function ($post_infos = [], $form_data = []) {
        $post_id = absint($post_infos['post_id'] ?? 0);
        if ($post_id <= 0) {
            return;
        }
        $wc_link = absint($post_infos['link_wc_id'] ?? ABPET_Function::get_post_info($post_id, 'link_wc_id', 0));
        if ($wc_link <= 0) {
            return;
        }
        ...
        <div class="total_continue_area">
            <div class="total_continue item_box_1">
                ...
                <?php } ?>
                <button class="_btn_light_theme book_continue" type="button" data-alert="<?php esc_attr_e('No Ticket Selected ! Please Select Ticket', 'abp-event-ticket'); ?>" data-msg="<?php esc_attr_e('Added to Cart Successfully', 'abp-event-ticket'); ?>">
                    <?php esc_html_e('Continue', 'abp-event-ticket'); ?>
