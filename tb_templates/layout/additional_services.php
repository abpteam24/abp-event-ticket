        ...
        if (!empty($additional_services) && is_array($additional_services)) {
            ...
            <div class="additional_service_area">
                <div class="item_box_1 additional_service">
                    <h5 class="_abp_title"><?php esc_html_e('Additional services ( Optional ) : ', 'abp-event-ticket'); ?></h5>
                    <?php foreach ($additional_services as $id => $service) {
                        ...
                        $name = $service['name'] ?? '';
                        $price = $service['price'] ?? '';
                        $tax_price = (!empty($price) && $price > 0) ? ABPET_Function::tax_with_price($post_id, $price) : 0;
                        $qty = $service['qty'] ?? '';
                        ...
                        if ($qty < 9999) {
                            $post_infos['ex_id'] = $id;
                            $sold_qty = ABPET_Query::get_sold_qty_ex($post_infos);
                        }
                            ...
                            <div class="_fd_column_w_full">
                                <div class="_fj_between">
                                    ...
                                    <?php if ($available > 0) { ?>
                                        ...
                                        <span class="_color_warning"><?php esc_html_e('Not Available !', 'abp-event-ticket'); ?></span>
                                    <?php } ?>
                                </div>
                                <h5 class="_abp_color_theme">
                                    <?php
                                        ...
                                        if ($returnable === 'yes') { ?>
                                            <span class="trash"><?php esc_html_e('Returnable', 'abp-event-ticket'); ?></span>
                                        <?php } ?>
                                </h5>
