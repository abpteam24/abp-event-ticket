let abpet_ticket_type = abpet_sp_config.seat_type ? JSON.parse(abpet_sp_config.seat_type) : null;
let abpet_decor_item = abpet_sp_config.decor_item ? JSON.parse(abpet_sp_config.decor_item) : null;
(function ($) {
    'use strict';
    let seat_groups = [
    ...
    abpet_ticket_type = (abpet_ticket_type && abpet_ticket_type.length) ? abpet_ticket_type : seat_groups;
    let decor_items = [
    ...
    abpet_decor_item = (abpet_decor_item && abpet_decor_item.length) ? abpet_decor_item : decor_items;
    let activeGroup = '';
    ...
    /**********************************/
    let sp_parent = abpet_parent.find('#abpet_sp_builder');
    let initialLayout = sp_parent.find('#sp_saved_data').data('layout') || [];
    window.abpet_sp_add = function (id = '', clone = '') {
        id = id !== '' ? parseInt(id) : '';
        let target = sp_parent.find('.sp_builder_area');
        $.ajax({
            type: 'POST', url: abpet_sp_config.ajax_url, data: {
                action: 'abpet_add_sp', id: id, clone: clone, 'nonce': abpet_sp_config.nonce
            }, beforeSend: function () {
                abpet_spinner(target);
                abpet_toast_msg(abpet_sp_config.msg.sp_loading);
    ...
    window.abpet_sp_clear = function () {
        if (confirm(abpet_sp_config.msg.sp_clear_confirm)) {
            initialLayout = [];
            ...
            generateGrid();
            abpet_toast_msg(abpet_sp_config.msg.sp_clear)
        }
    };
    window.abpet_sp_delete = function (id) {
        if (confirm(abpet_sp_config.msg.sp_delete_confirm)) {
            id = id !== '' ? parseInt(id) : '';
            let parent = sp_parent.find('.sp_list');
            $.ajax({
                type: 'POST', url: abpet_sp_config.ajax_url, data: {
                    "action": "abpet_delete_sp", 'id': id, 'nonce': abpet_sp_config.nonce
                }, beforeSend: function () {
                    abpet_spinner(parent);
                    abpet_toast_msg(abpet_sp_config.msg.sp_deleting, 'error');
                }, success: function (response) {
    ...
    window.abpet_sp_save = function () {
        ...
        let payload = {
            ...
            nonce: abpet_sp_config.nonce
        };
        $.ajax({
            type: 'POST', url: abpet_sp_config.ajax_url, data: payload,
            beforeSend: function () {
                abpet_spinner(sp_parent);
                abpet_toast_msg(abpet_sp_config.msg.sp_saving, 'info');
            }, success: function (response) {
                abpet_spinner_remove(sp_parent);
                abpet_toast_msg(response.data.msg, response.data.type);
                window.location.reload();
            }
        });
    };
    window.abpet_sp_init = function () {
    ...
    window.abpet_sp_row_column = function () {
        syncLayoutFromDOM();
        generateGrid();
    };
    window.abpet_sp_row_last_remove = function () {
        let rows = parseInt(sp_parent.find('.sp_rows').val()) || 10;
        if (rows <= 1) return;
        syncLayoutFromDOM();
    ...
    window.abpet_sp_col_last_remove = function () {
        let cols = parseInt(sp_parent.find('.sp_cols').val()) || 10;
        if (cols <= 1) return;
        syncLayoutFromDOM();
    ...
    window.abpet_sp_cell_wh = function () {
    ...
    window.abpet_sp_cell_design = function () {
    ...
    sp_parent.on('drop', '.sp_cell', function (e) {
        ...
        if (type === 'seat') {
            let activeSeatGroup = abpet_ticket_type.find(g => String(g.id) === String(groupId));
    ...
    function cell_data(type, id, key = 'icon') {
        let list = (type === 'seat') ? abpet_ticket_type : abpet_decor_item;
    ...
    function updateLiveCounters() {
        ...
        let groupCounts = {};
        let otherCounts = {};
        abpet_ticket_type.forEach(g => groupCounts[g.id] = 0);
        abpet_decor_item.forEach(g => otherCounts[g.id] = 0);
        sp_parent.find('.sp_cell').each(function () {
