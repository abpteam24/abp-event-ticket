window.abpet_parent = window.abpet_parent || jQuery('div.abpet_area');
function abpet_init(target = abpet_parent) {
    abpet_load_tabs(target);
    abpet_load_more(target);
    abpet_load_image(target);
    abpet_toast_init();
    abpet_required(target);
    abpet_load_datepicker(target);
    abpet_init_dynamic_date_pickers();
    if (typeof abpet_admin_init === 'function') {
        abpet_admin_init(target);
    }
    return true;
}
function abpet_load_tabs(parent = abpet_parent) {
    if (parent.find('.abp_tabs').length === 0) return;
    parent.find('.abp_tabs').each(function () {
...
function abpet_load_more(parent = abpet_parent) {
...
function abpet_load_image(parent = abpet_parent) {
...
function abpet_load_datepicker(parent = abpet_parent) {
    if (parent.find(".abp_datepicker:visible").length === 0) return;
    parent.find(".abp_datepicker:visible").each(function () {
        jQuery(this).removeClass('hasDatepicker').attr('id', '').removeData('datepicker').unbind();
    }).promise().done(function () {
        parent.find(".abp_datepicker:visible").datepicker({
            dateFormat: abpet_var.date_format, autoSize: true, changeMonth: true, changeYear: true, //showButtonPanel: true,
...
function abpet_init_dynamic_date_pickers(newSelector = null, newConfig = null) {
    window.abpet_picker_data = window.abpet_picker_data || {};
    if (newSelector && newConfig) {
        window.abpet_picker_data[newSelector] = newConfig;
    }
    if (jQuery.isEmptyObject(window.abpet_picker_data)) {
        return;
    }
    let currentDateFormat = (typeof abpet_var !== 'undefined' && abpet_var.date_format) ? abpet_var.date_format : 'yy-mm-dd';
    jQuery.each(window.abpet_picker_data, function (selector, config) {
...
function abpet_toast_init() {
    if (abpet_parent.find('.toast_msg_area').length === 0) {
        abpet_parent.first().append('<div class="toast_msg_area"></div>');
    }
    let toast_notices = abpet_parent.find('.toast_notice');
    if (toast_notices.length > 0) {
        toast_notices.each(function (index) {
            let current_notice = jQuery(this);
            let type = current_notice.attr('data-type') || 'info';
            let msg = current_notice.html();
            setTimeout(function () {
                abpet_toast_msg(msg, type);
            }, index * 600);
        });
    }
}
function abpet_required(parent = abpet_parent) {
    if (parent.find('[data-required]:visible').length === 0) return;
    parent.find('[data-required]:visible').each(function () {
        let target = jQuery(this);
...
//==============================================================================================//
function abpet_alert($this, attr = 'alert') {
    alert($this.data(attr));
}
function abpet_page_scroll(target) {
    jQuery('html, body').animate({
        scrollTop: target.offset().top -= 150
    }, 1000);
}
function abpet_toast_msg(msg, type = 'info') {
    const icons = {success: '✅', error: '❌', warn: '⚠️', info: 'ℹ️'};
    const el = jQuery(`<div class="toast_msg_box ${type}"><span>${icons[type] || 'ℹ️'}</span><span>${msg}</span></div>`);
    abpet_parent.find('.toast_msg_area').append(el).hide().fadeIn(200);
    setTimeout(() => el.fadeOut(300, () => el.remove()), 3400);
}
function abpet_wc_price_format(price) {
    if (typeof price === 'string') {
        price = Number(price);
    }
    price = price.toFixed(abpet_var.decimal_num);
    let total_part = price.toString().split(".");
    total_part[0] = total_part[0].replace(/\B(?=(\d{3})+(?!\d))/g, abpet_var.thousands_separator);
    price = total_part.join(abpet_var.currency_decimal);
    let price_text = '';
    if (abpet_var.currency_position === 'right') {
        price_text = price + abpet_var.currency_symbol;
    } else if (abpet_var.currency_position === 'right_space') {
        price_text = price + '&nbsp;' + abpet_var.currency_symbol;
    } else if (abpet_var.currency_position === 'left') {
        price_text = abpet_var.currency_symbol + price;
    } else {
        price_text = abpet_var.currency_symbol + '&nbsp;' + price;
    }
    if (abpet_var.currency_suffix) {
        price_text = price + '&nbsp;' + abpet_var.currency_suffix;
    }
    return price_text;
}
function abpet_spinner(parent = abpet_parent) {
    if (parent.find('.abp_spinner').length < 1) {
        parent.addClass('_p_relative').append('<div class="abp_spinner"></div>');
    }
}
function abpet_spinner_remove(parent = abpet_parent) {
...
function abpet_ajx_error(xhr, target) {
    abpet_spinner_remove(target);
...
(function ($) {
    ...
    $(document).ready(function () {
        $('body').find('div.abpet_area [data-image-href]').each(function () {
            abpet_spinner($(this));
        });
        abpet_init();
    });
...
function abpet_data_change($this) {
    abpet_load_image();
...
function abpet_icon_change(currentTarget) {
...
function abpet_text_change(currentTarget) {
...
function abpet_class_change(currentTarget) {
...
function abpet_input_value_change(currentTarget) {
...
document.body.addEventListener('click', function (event) {
    const targetDiv = event.target.closest('div.abpet_area [data-href]');
    if (targetDiv) {
...
window.abpet_date_close = function (element, e) {
...
function abpet_target_close(close_id, $this) {
...
function abpet_target_open(close_id, $this) {
    if ($this.closest('.tf_close_area').length > 0) {
...
function abpet_popup_close(target_id = '') {
    if (target_id) {
...
(function ($) {
    ...
    function get_collapse_target(current, id = '') {
        ...
        if (current.closest('.data_single_collapse').length > 0) {
            ...
            return abpet_parent.find('[data-collapse="' + target_id + '"]');
        }
    }
            ...
            $(parent_target_close).find('[data-collapse]').each(function () {
                if ($(this).hasClass('abp_active')) {
                    ...
                    if (collapse_id !== currentTarget.data('collapse-target')) {
                        ...
                        abpet_text_change(target_collapse);
                        abpet_icon_change(target_collapse);
                    }
                }
            })
        }
...
(function ($) {
    "use strict";
    //==============================================================================Qty inc dec================//
    $(document).on("click", "div.abpet_area .qty_decrease ,div.abpet_area .qty_increase", function () {
        let current = $(this);
    ...
    abpet_parent.on('abp_role_back', '.custom_checkbox [data-checked]', function () {
        let $this = $(this);
        $this.toggleClass('abp_active').promise().done(function () {
            let parent = $(this).closest('.custom_checkbox');
            let value = '';
        ...
        if (!$this.hasClass('abp_active')) {
            ...
            parent.find('.abp_active[data-radio]').each(function () {
                if ($(this).attr('data-close-target')) {
                    ...
                    abpet_target_close(close_id, $this);
                }
                ...
                if ($this.attr('data-close-target')) {
                    let close_id = $this.attr('data-close-target');
                    abpet_target_open(close_id, $this);
                }
    ...
    //==============================================================================custom select ================//
    $(document).on("click", "div.abpet_area .abp_dropdown .dropdown_list li", function (e) {
        e.preventDefault();
        let current = $(this);
    ...
    }, 'div.abpet_area .abp_dropdown input[type="text"]');
}(jQuery));
//================================================================================Filter and pagination=================//
function abpet_filter(parent) {
...
function abpet_pagination_item(parent) {
    let is_filter_active = parent.find('.pagination_item.abp_on, .pagination_item.abp_off').length > 0;
    let hidden_items;
    let total_items;
    if (!is_filter_active) {
...
(function ($) {
    ...
    abpet_parent.on('abp_trigger', '.abp_pagination [name="cat_id"]', function () {
        let parent = $(this).closest('.abp_pagination');
        abpet_filter(parent);
    });
    abpet_parent.on('change', '.abp_pagination [name="loc_id"]', function () {
        let parent = $(this).closest('.abp_pagination');
        abpet_filter(parent);
    });
    abpet_parent.on('abp_trigger', '.abp_pagination [name="loc_id"]', function () {
        let parent = $(this).closest('.abp_pagination');
        abpet_filter(parent);
    });
    $(document).on('click', 'div.abpet_area .grid_view', function () {
        ...
        if (container) {
            ...
            setTimeout(function () {
                container.removeClass('abpet_lists').addClass('abpet_grid');
    ...
    $(document).on('click', 'div.abpet_area .list_view', function () {
        ...
        if (container) {
            container.addClass('view-switching');
            setTimeout(function () {
                container.removeClass('abpet_grid').addClass('abpet_lists');
                container.removeClass('view-switching');
                abpet_load_more();
            }, 150);
        }
...
(function ($) {
    "use strict";
    $(document).on('click', '.abpet_slider [data-target-popup]', function () {
        let target = $(this).data('target-popup');
        $('body').addClass('_stop_scroll').find('[data-popup="' + target + '"]').addClass('in').promise().done(function () {
            abpet_load_image();
        });
    });
    $(document).on('click', '.abpet_slider .popup_close', function () {
        $(this).closest('[data-popup]').removeClass('in');
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('div.abpet_area [data-slider]').forEach(el => new ABPETSlider(el));
    const gallery_item = document.querySelectorAll('div.abpet_area .gallery_item');
    ...
    const faq_item = document.querySelectorAll('div.abpet_area .faq_item');
