<?php
    ...
    if (!class_exists('ABPET_Form')) {
        class ABPET_Form {
            public function __construct() {
                add_action('abpet_global_client_form', array($this, 'global_client_form'));
                add_action('abpet_post_content', [$this, 'post_client_form']);
                add_filter('abpet_get_form_array', array($this, 'get_form_array'));
                add_action('wp_ajax_abpet_save_form_config', array($this, 'save_form_config'));
                add_action('wp_ajax_abpet_import_client_form_content', array($this, 'import_client_form_content'));
            }
            ...
            public function post_client_form($post_infos): void {
                if (ABPET_Function::on_off('client_info')) {
                    $client_forms = $post_infos['abpet_form'] ?? [];
                    ...
                    <div class="tab_item abpet_client_form" data-tabs="#abpet_client_form">
                            ...
                            <div class="group_setting">
                                <div class="setting_item">
                                    <div class="_f_wrap_fj_between_fa_center">
                                        <label>
                                            <?php ABPET_Layout::switch_checkbox('display_client_form', $display); ?>
                                            <span><?php esc_html_e('Client Form ?', 'abp-event-ticket'); ?></span>
                                        </label>
                                    </div>
                                    <div class="_divider_xs"></div>
                                    <?php ABPET_Layout::info_text('display_client_form'); ?>
                                </div>
                                <div data-collapse="#display_client_form" class="setting_item <?php echo esc_attr($display == 'on' ? 'abp_active' : ''); ?>">
                                    <div class="_fj_between">
                                        <label>
                                            <?php ABPET_Layout::switch_checkbox('active_global_form', $active_global_form); ?>
                                            <span><?php esc_html_e('Use Global Client Form ?', 'abp-event-ticket'); ?></span>
                                        </label>
                                        <div data-collapse="#active_global_form" class=" <?php echo esc_attr($active_global_form == 'on' ? '' : 'abp_active'); ?>">
                                            <button type="button" class="_btn_theme" onclick="abpet_import_global('client_form_content')"><span class="fas fa-file-upload _mar_r_xs"></span><?php esc_html_e('Import Global Client Form', 'abp-event-ticket'); ?></button>
                                        </div>
                                    </div>
                                    <div class="_divider_xs"></div>
                                    <?php ABPET_Layout::info_text('active_global_form'); ?>
                                </div>
                                <?php if (ABPET_Function::on_off('same_attendee')) { ?>
                                    <div data-collapse="#display_client_form" class="setting_item <?php echo esc_attr($display == 'on' ? 'abp_active' : ''); ?>">
                                        <label>
                                            <?php ABPET_Layout::switch_checkbox('display_single_form', $display_single_form); ?>
                                            <span><?php esc_html_e('Same Attendee ?', 'abp-event-ticket'); ?></span>
                                        </label>
                                        <div class="_divider_xs"></div>
                                        <?php ABPET_Layout::info_text('display_single_form'); ?>
                                    </div>
                                <?php } else { ?>
                        ...
                        <?php } else {
                            ABPET_Layout::layout_warning_info_xs('attendee_off');
                        } ?>
                    </div>
                    <?php
                }
            ...
            public function passenger_form_settings($passenger_forms, $global = false): void {
                ...
                <div class="configuration_content _mar_t_xs">
                    <div class="_ov_auto">
                        <table class=" _abp">
                            ...
                            <tr>
                                ...
                                <th class="_text_table_center"><?php esc_html_e('Unique ID', 'abp-event-ticket'); ?><sup class="_color_required">*</sup></th>
                                <th class="_text_table_center"><?php esc_html_e('Form Type', 'abp-event-ticket'); ?><sup class="_color_required">*</sup></th>
                                <th class="_text_table_center">
                                    <?php esc_html_e('Value Option', 'abp-event-ticket'); ?><sup class="_color_required">*</sup>
                                    <?php ABPET_Layout::info_text('client_form_option'); ?>
                                </th>
                                <th class="_text_table_center"><?php esc_html_e('Default Value', 'abp-event-ticket'); ?></th>
                                <th class="_w_100_text_table_center"><?php esc_html_e('Required', 'abp-event-ticket'); ?><sup class="_color_required">*</sup></th>
                                <th class="_w_75_text_table_center"><?php esc_html_e('Action', 'abp-event-ticket'); ?></th>
                            </tr>
                            </thead>
                    ...
                    <div class="_divider_xs"></div>
                    <div class="_fj_between">
                        <?php ABPET_Layout::button_add(__('Add New Form Item', 'abp-event-ticket'));
                            if ($global) {
                                ABPET_Layout::button_global_save('form_config', __('Save Global Client Form Configuration', 'abp-event-ticket'));
                            } ?>
                    </div>
            ...
            public function form_item($form = [], $id = ''): void {
                ...
                <tr class="delete_area data_single_collapse">
                    <td>
                        <label>
                            <input type="text" class="_form_control_min_150 validation_name" name="client_form_title[]" placeholder="<?php esc_attr_e('Name', 'abp-event-ticket'); ?>" value="<?php echo esc_attr($label); ?>"/>
                        </label>
                    </td>
                    <th class="_text_table_center">
                        <?php if ($id) { ?>
                        ...
                        <?php } else { ?>
                            <label>
                                <input type="text" class="_form_control_min_150 validation_id" name="client_form_id[]" placeholder="<?php esc_attr_e('Unique ID', 'abp-event-ticket'); ?>" value="<?php echo esc_attr($id); ?>"/>
                            </label>
                        <?php } ?>
                    </th>
                    <td>
                        <label>
                            <select class="_form_control_min_150" name="client_form_type[]" data-collapse-target data-collapse-target-multi>
                                <option value="text" data-option-target-multi="#client_form_value" <?php echo esc_attr($type == 'text' ? 'selected' : ''); ?>><?php esc_html_e('Text', 'abp-event-ticket'); ?></option>
                                <option value="email" data-option-target-multi="#client_form_value" <?php echo esc_attr($type == 'email' ? 'selected' : ''); ?>><?php esc_html_e('E-Mail', 'abp-event-ticket'); ?></option>
                                <option value="number" data-option-target-multi="#client_form_value" <?php echo esc_attr($type == 'number' ? 'selected' : ''); ?>><?php esc_html_e('Number', 'abp-event-ticket'); ?></option>
                                <option value="select" data-option-target-multi="#client_form_type #client_form_value" <?php echo esc_attr($type == 'select' ? 'selected' : ''); ?>><?php esc_html_e('Select', 'abp-event-ticket'); ?></option>
                                <option value="checkbox" data-option-target-multi="#client_form_type #client_form_value" <?php echo esc_attr($type == 'checkbox' ? 'selected' : ''); ?>><?php esc_html_e('Checkbox', 'abp-event-ticket'); ?></option>
                                <option value="radio" data-option-target-multi="#client_form_type #client_form_value" <?php echo esc_attr($type == 'radio' ? 'selected' : ''); ?>><?php esc_html_e('Radio', 'abp-event-ticket'); ?></option>
                                <option value="textarea" data-option-target-multi="#client_form_value" <?php echo esc_attr($type == 'textarea' ? 'selected' : ''); ?>><?php esc_html_e('Textarea', 'abp-event-ticket'); ?></option>
                                <option value="date" data-option-target-multi="#client_form_type_date" <?php echo esc_attr($type == 'date' ? 'selected' : ''); ?>><?php esc_html_e('Date', 'abp-event-ticket'); ?></option>
                            </select>
                        </label>
                    </td>
                    <td>
                        <label data-collapse="#client_form_type" class="<?php echo esc_attr($active_type); ?>">
                            <input type="text" class="_form_control_min_150 validation_name" name="client_form_option[]" placeholder="<?php esc_attr_e('Value Option', 'abp-event-ticket'); ?>" value="<?php echo esc_attr($options); ?>"/>
                        </label>
                    </td>
                    <td>
                        <div class="<?php echo esc_attr($active_value); ?>" data-collapse="#client_form_value">
                            <label>
                                <input type="text" class="_form_control_min_150 validation_name" name="client_form_value[]" placeholder="<?php esc_attr_e('Default Value', 'abp-event-ticket'); ?>" value="<?php echo esc_attr($d_value); ?>"/>
                            </label>
                        </div>
                    ...
                    <td>
                        <?php ABPET_Layout::switch_checkbox('client_form_required[]', $required); ?>
                    </td>
            ...
            public function save_form_config(): void {
                ...
                update_option('abpet_form', $form_infos);
                wp_send_json_success(['msg' => __('Client Form Configuration Saved Successfully..... !! ', 'abp-event-ticket'), 'type' => 'success']);
            }
            public function import_client_form_content(): void {
                if (!check_ajax_referer('abpet_admin_ajax_nonce', 'nonce', false) || !current_user_can('manage_options')) {
                    wp_send_json_error(['msg' => __('Invalid security token or Insufficient permissions.', 'abp-event-ticket'), 'type' => 'warn'], 403);
                }
                $default_form = ABPET_Static::form();
                $forms = ABPET_Function::get_option('abpet_form', $default_form) ?? [];
                $forms = is_array($forms) ? $forms : [];
                ob_start();
                $this->passenger_form_settings($forms);
                $html_content = ob_get_clean();
                wp_send_json_success(['html' => $html_content, 'msg' => __('Global Client Form Imported Successfully ..... !! ', 'abp-event-ticket'), 'type' => 'success']);
            }
        }
        new ABPET_Form();
    }