<?php
	...
	if ( ! class_exists( 'ABPET_Hidden_Post' ) ) {
		class ABPET_Hidden_Post {
			...
			public function insert_wc_hidden_post( $post_id, $post ): void {
				...
				if ( $post->post_type === $cpt && $post->post_status === 'publish' ) {
					if ( empty( ABPET_Function::get_post_info( $post_id, 'exit_wc_hidden_post' ) ) ) {
			...
			public function save_hidden_post( $post_id ): void {
				...
				if ( $this->count_hidden_post( $post_id ) === 0 || empty( ABPET_Function::get_post_info( $post_id, 'link_wc_id' ) ) ) {
					$this->create_wc_hidden_post( $post_id, $title );
				}
				$product_id = ABPET_Function::get_post_info( $post_id, 'link_wc_id', $post_id );
				if ( ! $product_id ) {
					return;
				}
			...
			public function hide_hidden_post_frontend(): void {
				...
				if ( is_array( $visibility ) && ! empty( $visibility ) ) {
					if ( $visibility[0]->slug === 'exclude-from-catalog' || $visibility[0]->name === 'exclude-from-catalog' ) {
						$check_event_hidden = (int) ABPET_Function::get_post_info( $post->ID, 'abpet_link_id', 0 );
			...
			public function create_wc_hidden_post( $post_id, $title ): void {
				...
				$product->update_meta_data( 'abpet_link_id', $post_id );
				$pid = $product->save();
				if ( $pid ) {
					wp_set_object_terms( $pid, (int) $post_id, 'abpet_link_taxonomy' );
					...
					wp_cache_delete( 'abpet_count_hidden_post_' . $post_id, 'abpet_counts' );
					delete_transient( 'abpet_count_hidden_post_' . $post_id );
				}
			}
			public function count_hidden_post( $post_id ): int {
				$cache_group = 'abpet_counts';
				$cache_key   = 'abpet_count_hidden_post_' . $post_id;
				...
				if ( false === $count ) {
					...
					if ( false === $count ) {
						...
						$count = (int) $wpdb->get_var( $wpdb->prepare(
							"SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = 'abpet_link_id' AND meta_value = %s",
							(string) $post_id
						) );
			...
			public function exclude_url_from_search_engine(): void {
				...
				if ( ! empty( $visibility ) && is_object( $visibility[0] ) ) {
					if ( $visibility[0]->slug === 'exclude-from-catalog' || $visibility[0]->name === 'exclude-from-catalog' ) {
						$check_hidden = (int) ABPET_Function::get_post_info( $post->ID, 'abpet_link_id', 0 );
						if ( $check_hidden > 0 ) {
			...
			public function get_all_hidden_product_id(): array {
				$product_ids = [];
				$query       = ABPET_Query::query_post_type( ABPET_Function::get_cpt() );
				if ( isset( $query->posts ) && is_array( $query->posts ) ) {
					foreach ( $query->posts as $result ) {
						$wc_id = ABPET_Function::get_post_info( $result->ID, 'link_wc_id' );
						if ( $wc_id ) {
